...
def main():
    ...
    (site / "whl").mkdir(parents=True, exist_ok=True)

    # One PEP 503 sub-index per line: whl/<line>/tnx/index.html
    for line, wheels in buckets.items():
        pkg_dir = site / "whl" / line / PACKAGE
        ...
        (site / "whl" / line / "index.html").write_text(
            _simple_page([f'    <a href="{PACKAGE}/">{PACKAGE}</a><br/>'])
        )

    # Human-readable landing page listing the available lines.
    line_links = "\n".join(
        f'    <a href="{line}/">{line}</a> ({len(w)} wheels)<br/>'
        for line, w in sorted(buckets.items())
    )
    (site / "whl" / "index.html").write_text(
        "<!DOCTYPE html>\n<html><body>\n"
    ...
    (site / "index.html").write_text(
        '<!DOCTYPE html>\n<html><head>'
        '<meta http-equiv="refresh" content="0; url=whl/">'
        "</head></html>\n"
    )
